...
import { appendChildrenToList } from "../galleries/galleries.validation"
...
const listGalleries = async (req: Request, res: Response) => {
	try {
		const response: IGallery[] = res.locals.foundGalleries
		res.json(response)
	} catch (err) {
		errorHandler(err, res)
	}
...
export const UsersController = {
	create: [isValidUser, appendData, create],
	read: [userExists, appendChildren, read],
	update: [userExists, isValidUser, update],
	delete: [userExists, destroy],
	readByEmail: [readByEmail, userExists, appendChildren, read],
	listGalleries: [galleriesExists, appendChildrenToList, listGalleries],
}